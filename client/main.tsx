...
function App() {
  ...
  const mapEmployee=async(employeeId:string)=>{if(!selected)return;await fetch('/api/admin/map',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({eventId:selected.id,employeeId})});await fetch(`/api/admin/events/${selected.id}/retry`,{method:'POST'});setSelected(null);void load()};
  ...
  return <main>
    ...
    <section className="panel"><div className="toolbar"><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search employee, door, or reason"/><select value={status} onChange={event=>setStatus(event.target.value)}>{statuses.map(value=><option key={value} value={value}>{value||'All statuses'}</option>)}</select></div><div className="table"><table><thead><tr><th>Event time</th><th>Person</th><th>Movement</th><th>Reader</th><th>Status</th><th>Decision</th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td>{local(row.occurredAt)}<small>Received {local(row.receivedAt)}</small></td><td>{row.employeeName||row.name||'Unknown'}{row.status==='held'&&<button className="link" onClick={()=>{setSelected(row);setEmployeeQuery('')}}>Enter employee email</button>}</td><td><b className={`direction ${row.direction}`}>{row.direction?.toUpperCase()||'—'}</b></td><td>{row.door||'—'}</td><td><span className={`badge ${row.status}`}>{row.status}</span></td><td>{row.reason}<small>Event #{row.id}</small></td></tr>)}</tbody></table>{!rows.length&&<div className="empty">No events match this view.</div>}</div></section>
    {selected&&<div className="overlay" onClick={()=>setSelected(null)}><section className="picker" onClick={event=>event.stopPropagation()}><h2>Enter employee email</h2><p>Automatic matching uses email only. Enter the employee email from Jisr and select the result.</p><input autoFocus value={employeeQuery} onChange={event=>setEmployeeQuery(event.target.value)} placeholder="Employee email"/><div className="employees">{employees.map(employee=><button key={employee.id} onClick={()=>void mapEmployee(employee.id)}><strong>{employee.email}</strong><span>{employee.name} · Code {employee.code}</span></button>)}</div><button className="cancel" onClick={()=>setSelected(null)}>Cancel</button></section></div>}
  </main>;
}
